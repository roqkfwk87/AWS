...
import com.roqkfwk.basic.entity.CustomOAuth2User;
...
public class OAuth2UserServiceImplement extends DefaultOAuth2UserService {
  ...
  // loadUser 메서드 :
  // - OAuth2.0 인증 성공 시에 인증 서버로부터 받은 request 객체를 사용하여 해당 서비스에 클라이언트의 정보를 로드하는 작업
  @Override
    ...
    try {
      ...
      exception.printStackTrace(); // 인증 받은 서버이름
    }
    
    OAuth2User oAuth2User2 = super.loadUser(userRequest);

    String name = (String) oAuth2User.getAttributes().get("login"); // 클라이언트 정보를 출력
