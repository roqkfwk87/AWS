/*
    if else 문
    - 조건식이 true일 경우 if 코드 블럭 실행, false일 경우 else코드 블럭 실행
...
// 0.5보다 큰면 무한히 루프를 반복, 값이 0.5 이하로 내려가면 루프가 멈춤
...
/*
    continue 문
    - 반복문을 진행 위치를 맨마지막으로 이동시킴 
*/
// count = 0;
// while (true) {
// count = 0이므로 무한루프에 빠지기 때문에 잘 못 된 코드임 
...
while (true) {
    if (count === 10) break;
// 짝수일때 증가 홀수일땐 증가하지 않음 
// 짝수는 증가하여 홀수값이 출력, 홀수는 그대로 유지하기 때문에 결과값이 홀수값이 나옴  
    if(count % 2 === 0) {
    count++; 
    continue;
    }
